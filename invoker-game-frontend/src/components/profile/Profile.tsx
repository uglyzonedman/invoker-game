...
const Profile = () => {
	...
	return (
		<div className='max-w-[1280px]  w-full mt-6 mx-auto'>
			...
			<div className='max-w-xl bg-gray-800 shadow-lg rounded-lg mt-4 overflow-hidden border border-gray-700 hover:shadow-2xl transition-shadow duration-300 mx-auto flex flex-col p-6 text-gray-200'>
					...
					getAllHistory.map((history: any, index: number) => (
						...
						>
							...
							<span className='text-gray-300'>
								{history?.resultGame?.result} с
							</span>
							<span className='text-sm text-gray-400'>
								Мод - {history?.resultGame?.gameMode}
							</span>
						</div>
					))
				) : (
